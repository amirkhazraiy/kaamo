...
let AuthService = class AuthService {
    constructor(usersService, jwtService, config, refreshSessions) {
    ...
    async login(loginDto) {
        ...
        const passwordMatches = await bcrypt.compare(loginDto.password, user?.passwordHash ?? DUMMY_PASSWORD_HASH);
    ...
    async refresh(refreshToken) {
        ...
        const revoked = await this.refreshSessions.update({ id: session.id, revokedAt: (0, typeorm_2.IsNull)() }, {
            revokedAt: new Date(),
    ...
    async createSession(user, refreshToken = this.generateRefreshToken(), refreshTokenHash = this.hashRefreshToken(refreshToken)) {
...
exports.AuthService = AuthService = __decorate([
    ...
    __param(3, (0, typeorm_1.InjectRepository)(refresh_session_entity_1.RefreshSession)),
